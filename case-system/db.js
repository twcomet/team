...
_addCol('quote_sheet_items', 'sell_mode',      "TEXT DEFAULT ''"); // 膜料販售：'roll'=整支、其餘=散米/米
...
_addCol('quote_sheets', 'hide_marketing', 'INTEGER DEFAULT 0'); // 1=客戶版隱藏行銷優惠(同意拍照)整區(賣材料等不需要)
...
// 估價/客人照片歸檔到案件 Drive 資料夾的去重紀錄（同案件同來源網址只上傳一次，避免每次存檔重複）
db.exec(`CREATE TABLE IF NOT EXISTS est_photo_archive (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  case_id INTEGER,
  src_url TEXT,
  drive_file_id TEXT,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(case_id, src_url)
)`);
