...
router.get('/film-catalog', requireAuth, (req, res) => {
  const me = req.session.user;
  const rows = db.prepare(`SELECT brand, asia_code, kr_code, color, fireproof, per_m, ecom_price, cost_per_m, width, plane, cabinet, shape FROM est_film_catalog WHERE active=1 ORDER BY sort_order, id`).all();
