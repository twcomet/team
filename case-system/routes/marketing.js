...
router.get('/summary', requireAuth, (req, res) => {
  ...
  const monthlyFunnel = db.prepare(`
    SELECT strftime('%Y-%m', created_at) as month,
           COUNT(*) as total,
           SUM(CASE WHEN status IN ('survey','quoted','contracted','payment','closed') THEN 1 ELSE 0 END) as surveyed,
  ...
  res.json({ sourceStats, monthlyClients, monthlyFunnel, caseTypeStats, lineStats, keywords, fromDate, toDate });
});

// GET /api/marketing/cs-funnel?from=&to=&group=month|week|day&org_id=
// Returns case funnel stages per time period, plus per-user summary
router.get('/cs-funnel', requireAuth, (req, res) => {
  ...
  const periods = db.prepare(`
    SELECT strftime('${fmt}', c.created_at) as period,
           COUNT(*) as inquiries,
           SUM(CASE WHEN c.status IN ('survey','quoted','contracted','payment','closed') THEN 1 ELSE 0 END) as surveyed,
