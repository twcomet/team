...
function gatherDispatchSnapshot(me) {
  ...
  // 待排場勘
  const toSurvey = q(`SELECT c.case_number, c.title, cl.name client, c.desired_entry_date,
      CAST(julianday('now')-julianday(c.updated_at) AS INT) days
    FROM cases c LEFT JOIN clients cl ON cl.id=c.client_id
...
你會收到一份「排工現況快照」(JSON)，內容含：待派施工的成交案、待排場勘、未來 14 天的場勘/施工/維修排程、施工未指定小組長、各師傅工作量、以及同一師傅同一天被排多筆（可能衝突）。
