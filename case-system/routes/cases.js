...
router.put('/:id', requireAuth, (req, res) => {
  ...
  `).run(
    ...
    line_source ?? null, line_display_name ?? null, keyword ?? null, client_category ?? null,
...
router.delete('/:id', requireAuth, (req, res) => {
  ...
  try {
    ...
    db.prepare(`DELETE FROM revenue_shares      WHERE case_id=?`).run(id);
    db.prepare(`DELETE FROM material_orders     WHERE case_id=?`).run(id);
