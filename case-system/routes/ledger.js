...
router.post('/', requireAuth, (req, res) => {
  ...
  const { date, type, category, amount, case_id, description, org_id, hidden, pay_status, pay_due_date, paid_note, vendor, client_id, sub_category, brand, payee_info } = req.body;
  ...
  const r = db.prepare(`
    INSERT INTO ledger_entries (date, type, category, amount, case_id, description, org_id, created_by, hidden, pay_status, pay_due_date, paid_note, paid_at, vendor, client_id, sub_category, brand, payee_info)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(date, type, category, Number(amount), case_id || null, description || null, org_id || null, uid,
         hidden ? 1 : 0, pay_status || null, pay_due_date || null, paid_note || null, paid_at, vendor || null, client_id || null, sub_category || null, brand || null, payee_info || null);
...
router.put('/:id', requireAuth, (req, res) => {
  try {
    ...
    db.prepare(`
      UPDATE ledger_entries SET date=?, type=?, category=?, amount=?, case_id=?, description=?, org_id=?, hidden=?, pay_status=?, pay_due_date=?, paid_note=?, paid_at=?, vendor=?, client_id=?, sub_category=?, brand=?, payee_info=?
      WHERE id=?
    `).run(date, type, category, Number(amount), case_id || null, description || null, org_id || null,
           hidden ? 1 : 0, pay_status || null, pay_due_date || null, paid_note || null, paid_at, vendor || null, client_id || null, sub_category || null, brand || null, payee_info || null, req.params.id);
