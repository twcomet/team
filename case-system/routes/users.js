...
router.put('/:id', requireCanManageUsers, (req, res) => {
  ...
  const { name, role, department, is_manager, can_see_amounts, can_delete, service_areas, active, permissions, daily_cost, accept_dispatch } = req.body;
  ...
  db.prepare(`UPDATE users SET name=?, role=?, department=?, is_manager=?, can_see_amounts=?, can_delete=?, service_areas=?, active=?, permissions=?, daily_cost=?, allowed_org_ids=?, accept_dispatch=? WHERE id=?`)
