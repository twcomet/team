...
router.get('/', requireAuth, (req, res) => {
  ...
  let query = `SELECT u.id, u.name, u.username, u.role, u.org_id, u.department,
                      ...
                      u.permissions, u.sort_order, u.daily_cost, u.line_notify_token, o.name as org_name
...
router.put('/:id', requireCanManageUsers, (req, res) => {
  ...
  const { name, role, department, is_manager, can_see_amounts, service_areas, active, permissions, daily_cost, line_notify_token } = req.body;
  db.prepare(`UPDATE users SET name=?, role=?, department=?, is_manager=?, can_see_amounts=?, service_areas=?, active=?, permissions=?, daily_cost=?, line_notify_token=? WHERE id=?`)
    .run(name, role, department, is_manager ? 1 : 0, can_see_amounts ? 1 : 0,
         JSON.stringify(service_areas || []), active ?? 1,
         JSON.stringify(permissions || {}), daily_cost ?? null, line_notify_token ?? null, req.params.id);
