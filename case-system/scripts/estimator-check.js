...
// ── ⑪ DB 牌價路徑：buildCatalogFromDb 與 JS catalog 對拍（確認設定頁改價會生效、seed 一致）──
try {
  const db = require('../db');
  const dbCat = eng.buildCatalogFromDb(db);
  const carts = [
    [{ kind: 'film', brand: 'bodaq', idx: 1, work: 'plane', h: 250, w: 220 }],
    [{ kind: 'film', brand: 'benif', idx: 0, work: 'cabinet', h: 240, w: 300 }],
    [{ kind: 'door', cat: 'fire', origin: 'jp', side: 'double', size: 'small' }],
    [{ kind: 'door', cat: 'main', origin: 'kr', side: 'single', frame: 'yes' }],
    [{ kind: 'glass', cat: 'fog', idx: 0, h: 200, w: 100 }],
    [{ kind: 'elev-floor', ftype: 'one' }],
  ];
  carts.forEach((c, i) => {
    const a = eng.quote(c, { cust: 'owner', region: '雲嘉投' }, cat).total;
    const b = eng.quote(c, { cust: 'owner', region: '雲嘉投' }, dbCat).total;
    eq(b, a, `⑪DB牌價對拍#${i + 1}（JS=${a} / DB=${b}）`);
  });
